        ...
        <el-table style="width: 100%" border :data="spu.spuSaleAttrList">
          ...
          <el-table-column prop="prop" label="属性值名称列表" width="width">
            <template slot-scope="{ row, $index }">
              <!--  " -->
              <!-- el-tag:用户展示已有属性值列表的数据的    closable:是否可关闭      :disable-transitions 是否禁用渐变动画	-->
...
export default {
  ...
  data() {
    return {
      ...
      tradeMarkList: [], //存储品牌信息--------->tmId
      spuImageList: [], //存储SPU图片的数据----->spu.spuImageList
      saleAttrList: [], //销售属性的数据（项目全部的销售属性）
      attrIdAndAttrName: '' //收集未选择的销售属性的id----->解构--->spu.spuSaleAttrList
    }
  },
  computed: {
    //计算出还未选择的销售属性
