<template>
  <div>
    ...
    -->
    <el-pagination style="margin-top: 20px; text-align: center" :current-page="page" :total="total" :page-size="limit" :pager-count="7" :page-sizes="[3, 5, 10]" @current-change="getPageList" @size-change="handleSizeChange" layout="prev, pager, next, jumper,->,sizes,total"> </el-pagination>
    <!--
      对话框
      :visible.sync:控制对话框显示与隐藏用的
      .sync修饰符可以实现子组件与父组件的双向绑定，并且可以实现子组件同步修改父组件的值。---------------(语法糖)
      Form 组件提供了表单验证的功能，只需要通过 rules 属性传入约定的验证规则，并将 Form-Item 的 prop 属性设置为需校验的字段名即可
    -->
    <!-- 添加品牌回调清除数据了,修改品牌回调赋值数据了 -->
...
export default {
  ...
  methods: {
    ...
    updateTradeMark(row) {
      ...
      // ! 也就是tmForm存储  即为服务器返回品牌信息  (执行后this.tmForm上有 id)
      this.tmForm = { ...row }
    },
    ...
    //删除品牌的操作
    deleteTradeMark(row) {
      //弹框 ---------当进行一些操作时，有时需要弹出一些确定信息，一般有两种形式：提示框和确认框
